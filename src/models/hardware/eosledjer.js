...
var Eos = function () {
    ...
    (0, _createClass3.default)(Eos, [{
        ...
        value: function signTransaction(path, rawTxHex) {
            ...
            const config = store.default.getters.eosConfigLedger
            if (!config) {
              throw 'empty config for ledger'
            }
            const { fc } = EosApi({httpEndpoint: config.httpEndpoint, chainId:config.chainId});
            let b;
            try {
              b = serialize(config.chainId, rawTxHex.transaction, fc.types).toString('hex');
            } catch(e){
              console.error(e);
              return null;
            }
